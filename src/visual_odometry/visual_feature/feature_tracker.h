...
public:
    ...
    ros::Publisher pub_depth_feature_1;
    ros::Publisher pub_depth_feature_2;
    ros::Publisher pub_depth_image;
    ros::Publisher pub_depth_cloud;
    ros::Publisher pub_all_depth_cloud;
    ...
    n(n_in)
    {
        ...
        pub_depth_feature_1 = n.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/vins/depth/depth_feature_1", 5);
        pub_depth_feature_2 = n.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/vins/depth/depth_feature_2", 5);
        ...
        pub_all_depth_cloud = n.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/vins/depth/all_depth_cloud",   5);
                                          ...
                                          int cam_id)
    {
        ...
        double c_l_tx, c_l_ty, c_l_tz, c_l_rx, c_l_ry, c_l_rz;
        switch (cam_id)
        {
            case 0:
                {
                    c_l_tx = C_L_TX; c_l_ty = C_L_TY; c_l_tz = C_L_TZ;
                    c_l_rx = C_L_RX; c_l_ry = C_L_RY; c_l_rz = C_L_RZ;
                    break;
                }
            case 1:
                {
                    c_l_tx = C_L_TX1; c_l_ty = C_L_TY1; c_l_tz = C_L_TZ1;
                    c_l_rx = C_L_RX1; c_l_ry = C_L_RY1; c_l_rz = C_L_RZ1;
                    break;
                }
            case 2:
                {
                    c_l_tx = C_L_TX2; c_l_ty = C_L_TY2; c_l_tz = C_L_TZ2;
                    c_l_rx = C_L_RX2; c_l_ry = C_L_RY2; c_l_rz = C_L_RZ2;
                    break;
                }
            default:
                break;
        }
        Eigen::Quaterniond Q_cam2lidar;
        Q_cam2lidar = ::Eigen::AngleAxisd(c_l_rz, ::Eigen::Vector3d::UnitZ()) *
                      ::Eigen::AngleAxisd(c_l_ry, ::Eigen::Vector3d::UnitY()) *
                      ::Eigen::AngleAxisd(c_l_rx, ::Eigen::Vector3d::UnitX());
        Eigen::Vector3d t_cam2lidar (c_l_tx, c_l_ty,c_l_tz);
        ...
        for (int i = 0; i < (int)features_2d.size(); ++i)
        {
            ...
            // convert to ROS standard  lidar2cam
            Eigen::Vector3d new_feature_cur = Q_cam2lidar * feature_cur;
            PointType p;

            p.x = new_feature_cur(0);
            p.y = new_feature_cur(1);
            p.z = new_feature_cur(2);
            p.intensity = -1; // intensity will be used to save depth
            // the coordinate of cam's features in Lidar frame(z is meaningless now since it should represent depth)
            features_3d_sphere->push_back(p);
        }

        // 3. project depth cloud on a range image, filter points stacked in the same region
        ...
        pcl::PointCloud<PointType>::Ptr depth_cloud_all(new pcl::PointCloud<PointType>());
        ...
        for (int i = 0; i < (int)depth_cloud_local->size(); ++i)
        {
            ...
            float col_angle = atan2(abs(p.x), p.y) * 180.0 / M_PI; // degrees, left -> right, 0 -> 360
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        ...
        R_cam2lidar = Eigen::AngleAxisd(c_l_rz, Eigen::Vector3d::UnitZ()) *
                      Eigen::AngleAxisd(c_l_ry, Eigen::Vector3d::UnitY()) *
                      Eigen::AngleAxisd(c_l_rx, Eigen::Vector3d::UnitX());
        Eigen::Matrix4d T_cam2lidar;
        T_cam2lidar << R_cam2lidar(0,0), R_cam2lidar(0,1), R_cam2lidar(0,2), t_cam2lidar(0),
        ...
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            ...
            if (pointSearchInd.size() == 3 && pointSearchSqDis[2] < dist_sq_threshold)
            {
                ...
                visual_features_in_cam = T_cam2lidar.inverse() * visual_features_in_lidar;
                features_3d_sphere->points[i].intensity = visual_features_in_cam(2);
                // cout << "cam_id: " << cam_id << " " << "visual_features_in_cam: " << visual_features_in_cam(2) << endl;
                // over
