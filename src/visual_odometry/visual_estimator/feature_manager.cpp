...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, double td)
{
    ...
    for (auto &id_pts : image)
    {
        ...
        int cam_id = id_pts.second[0].first;
        ...
        if (it == feature.end())
        {
            ...
            feature.push_back(FeaturePerId(feature_id, frame_count, f_per_fra.depth, cam_id));
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        Eigen::Vector3d t0 = Ps[imu_i] + Rs[imu_i] * tic[it_per_id.cam_id];
        Eigen::Matrix3d R0 = Rs[imu_i] * ric[it_per_id.cam_id];
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            Eigen::Vector3d t1 = Ps[imu_j] + Rs[imu_j] * tic[it_per_id.cam_id];
            Eigen::Matrix3d R1 = Rs[imu_j] * ric[it_per_id.cam_id];
