...
ros::Publisher pub_point_cloud1, pub_margin_cloud1;
ros::Publisher pub_point_cloud2, pub_margin_cloud2;
ros::Publisher pub_key_poses;
ros::Publisher pub_camera_pose;
ros::Publisher pub_camera_pose1;
ros::Publisher pub_camera_pose2;
ros::Publisher pub_camera_pose_visual;
ros::Publisher pub_camera_pose_visual1;
ros::Publisher pub_camera_pose_visual2;
nav_msgs::Path path;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_point_cloud1        = n.advertise<sensor_msgs::PointCloud>          (PROJECT_NAME + "/vins/odometry/point_cloud_1", 1000);
    pub_margin_cloud1       = n.advertise<sensor_msgs::PointCloud>          (PROJECT_NAME + "/vins/odometry/history_cloud_1", 1000);
    pub_point_cloud2        = n.advertise<sensor_msgs::PointCloud>          (PROJECT_NAME + "/vins/odometry/point_cloud_2", 1000);
    pub_margin_cloud2       = n.advertise<sensor_msgs::PointCloud>          (PROJECT_NAME + "/vins/odometry/history_cloud_2", 1000);
    ...
    pub_camera_pose1        = n.advertise<nav_msgs::Odometry>               (PROJECT_NAME + "/vins/odometry/camera_pose_1", 1000);
    pub_camera_pose2        = n.advertise<nav_msgs::Odometry>               (PROJECT_NAME + "/vins/odometry/camera_pose_2", 1000);
    pub_camera_pose_visual  = n.advertise<visualization_msgs::MarkerArray>  (PROJECT_NAME + "/vins/odometry/camera_pose_visual", 1000);
    pub_camera_pose_visual1 = n.advertise<visualization_msgs::MarkerArray>  (PROJECT_NAME + "/vins/odometry/camera_pose_visual_1", 1000);
    pub_camera_pose_visual2 = n.advertise<visualization_msgs::MarkerArray>  (PROJECT_NAME + "/vins/odometry/camera_pose_visual_2", 1000);
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header, const int& failureId)
{
    ...
    tf::Quaternion q_imu_odom(Q.x(), Q.y(), Q.z(), Q.w());
    q_imu_odom.normalize();
    // q_imu_odom -->  imu2vins_world
    // vins_body = imu_link
    tf::Quaternion q_odom_ros = q_imu_odom * q_lidar_to_imu;
...
{
    if (pub_camera_pose_visual1.getNumSubscribers() == 0)
        return;
...
void pubCameraPose2(const Estimator &estimator, const std_msgs::Header &header)
{
    if (pub_camera_pose_visual2.getNumSubscribers() == 0)
        return;
...
{
    if (pub_point_cloud1.getNumSubscribers() != 0)
    {
        sensor_msgs::PointCloud point_cloud;
        point_cloud.header = header;
        point_cloud.header.frame_id = "vins_world";
    ...
    // pub margined potin
    if (pub_margin_cloud1.getNumSubscribers() != 0)
    {
        sensor_msgs::PointCloud margin_cloud;
        margin_cloud.header = header;
        margin_cloud.header.frame_id = "vins_world";
...
void pubPointCloud2(const Estimator &estimator, const std_msgs::Header &header)
{
    if (pub_point_cloud2.getNumSubscribers() != 0)
    {
        sensor_msgs::PointCloud point_cloud;
        point_cloud.header = header;
        point_cloud.header.frame_id = "vins_world";
    ...
    // pub margined potin
    if (pub_margin_cloud2.getNumSubscribers() != 0)
    {
        sensor_msgs::PointCloud margin_cloud;
        margin_cloud.header = header;
        margin_cloud.header.frame_id = "vins_world";
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    transform1.setOrigin(tf::Vector3(estimator.tic[1].x(),
                                    estimator.tic[1].y(),
                                    estimator.tic[1].z()));
    ...
    br.sendTransform(tf::StampedTransform(transform1, header.stamp, "vins_body", "vins_camera_1"));
