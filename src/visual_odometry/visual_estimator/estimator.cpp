...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, 
                             ...
                             const std_msgs::Header &header, int cam_id)
{
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            {
                result = initialStructure(cam_id);
...
void Estimator::processImage2(const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image,
                              ...
                              const std_msgs::Header &header)
{
    // Add new image features
    if (f_manager.addFeatureCheckParallax(frame_count, image, td) && 
        f_manager.addFeatureCheckParallax(frame_count, image1, td))
    {
        marginalization_flag = MARGIN_OLD;
    }
    ...
    // Marginalize old imgs if lidar odometry available for initialization
    if (solver_flag == INITIAL && lidar_initialization_info[0] >= 0 && lidar_initialization_info_1[0] >= 0)
        marginalization_flag = MARGIN_OLD;

    Headers[frame_count] = header;

    // cout << std::fixed << "before: Headers[" << frame_count << "]: " << Headers[frame_count].stamp.toSec() << endl;

    ImageFrame imageframe(image, lidar_initialization_info, header.stamp.toSec());
    ImageFrame imageframe1(image1, lidar_initialization_info_1, header.stamp.toSec());
...
void Estimator::processImage3(const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image,
                              const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image1,
                              ...
                              const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td) && 
        f_manager.addFeatureCheckParallax(frame_count, image1, td) &&
        f_manager.addFeatureCheckParallax(frame_count, image2, td))
    {
        marginalization_flag = MARGIN_OLD;
    }
    ...
    // Marginalize old imgs if lidar odometry available for initialization
    if (solver_flag == INITIAL && lidar_initialization_info[0] >= 0 && lidar_initialization_info_1[0] >= 0 && lidar_initialization_info_2[0] >= 0)
        marginalization_flag = MARGIN_OLD;
    ...
    ImageFrame imageframe(image, lidar_initialization_info, header.stamp.toSec());
    ImageFrame imageframe1(image1, lidar_initialization_info_1, header.stamp.toSec());
    ImageFrame imageframe2(image2, lidar_initialization_info_2, header.stamp.toSec());

    imageframe.pre_integration = tmp_pre_integration;
    imageframe1.pre_integration = tmp_pre_integration;
    imageframe2.pre_integration = tmp_pre_integration;
...
bool Estimator::initialStructure(int cam_id)
{
    // Lidar initialization
    {
        ...
        if (lidar_info_available == true)
        {
            ...
            Vector3d TIC_TMP[NUM_OF_CAM_ALL];
            for(int i = 0; i < NUM_OF_CAM; i++)
                TIC_TMP[i].setZero();
            ric[0] = RIC[0];
            f_manager.setRic(ric);
...
bool Estimator::visualInitialAlign(int cam_id)
{
    VectorXd x;
    //solve scale
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            if (ESTIMATE_TD)
            {
                ...
                problem.AddResidualBlock(f_td, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[cam_id], para_Feature[feature_index], para_Td[0]);
            ...
            {
                ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j, cam_id);
                problem.AddResidualBlock(f, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[cam_id], para_Feature[feature_index]);
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)
                {
                    ...
                    else
                    {
                        ...
                        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f, loss_function,
                                                                                       vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[cam_id], para_Feature[feature_index]},
                                                                                       vector<int>{0, 3});
                        marginalization_info->addResidualBlockInfo(residual_block_info);
                    }
                }
...
void Estimator::slideWindow(int cam_id)
{
    TicToc t_margin;
    if (marginalization_flag == MARGIN_OLD)
    {
    ...
    else
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            slideWindowNew(cam_id);
        }
    }
}

// real marginalization is removed in solve_ceres()
void Estimator::slideWindowNew(int cam_id)
{
    sum_of_front++;
    f_manager.removeFront(frame_count);
}
// real marginalization is removed in solve_ceres()
void Estimator::slideWindowOld(int cam_id)
{
    sum_of_back++;
