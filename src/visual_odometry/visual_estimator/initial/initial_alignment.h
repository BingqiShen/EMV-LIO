...
public:
    ...
    vector<float> getOdometry(deque<nav_msgs::Odometry>& odomQueue, double img_time, int cam_id)
    {
        tf::Quaternion q_cam_to_lidar;
        Eigen::Vector3d p_cam_to_lidar;
        if(cam_id == 0)
        {
            q_cam_to_lidar.setRPY(C_L_RX, C_L_RY, C_L_RZ);
            p_cam_to_lidar(0) = C_L_TX; p_cam_to_lidar(1) = C_L_TY; p_cam_to_lidar(2) = C_L_TZ;
        }
        else if(cam_id == 1)
        {
            q_cam_to_lidar.setRPY(C_L_RX1, C_L_RY1, C_L_RZ1);
            p_cam_to_lidar(0) = C_L_TX1; p_cam_to_lidar(1) = C_L_TY1; p_cam_to_lidar(2) = C_L_TZ1;
        }
        else if(cam_id == 2)
        {
            q_cam_to_lidar.setRPY(C_L_RX2, C_L_RY2, C_L_RZ2);
            p_cam_to_lidar(0) = C_L_TX2; p_cam_to_lidar(1) = C_L_TY2; p_cam_to_lidar(2) = C_L_TZ2;
        }
        ...
        for (int i = 0; i < (int)odomQueue.size(); ++i)
        {
            ...
            if (odomCur.header.stamp.toSec() < img_time - DELTA_T_IMU) // imu frequency
                continue;
            else
                break;
        }
        ...
        Eigen::Quaterniond q_odom_lidar_eigen(q_odom_lidar[3], q_odom_lidar[0], q_odom_lidar[1], q_odom_lidar[2]);

        tf::quaternionMsgToTF(odomCur.pose.pose.orientation, q_odom_lidar);

        tf::Quaternion q_odom_cam = tf::createQuaternionFromRPY(0, 0, M_PI) * (q_odom_lidar * q_cam_to_lidar); // global rotate by pi // mark: camera - lidar
        ...
        Eigen::Vector3d p_eigen_new = q_odom_lidar_eigen * p_cam_to_lidar + p_eigen;
